...
- Q: 聊天消息的存储策略是什么？ → A: 仅游戏期间内存缓存，游戏结束后清除
...
- **FR-021**: 系统必须阻止发送空消息
- **FR-022**: 系统必须在断线重连后加载最近的聊天历史（最多50条），聊天消息仅在游戏期间缓存，游戏结束后自动清除
...
- **FR-043**: 系统必须为所有 WebSocket 事件实现错误处理和重试机制，重连后接收完整游戏状态快照
- **FR-043a**: 系统必须记录错误和警告级别的日志，包括 API 调用失败、WebSocket 断线事件、游戏状态异常等
...
- **ChatMessage**: 聊天消息，包含发送者ID、发送者用户名、消息内容、发送时间戳、房间代码。消息仅在游戏期间缓存于内存，不持久化存储。
