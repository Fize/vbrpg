...
- Q: 注册时密码除了最小长度外，还需要满足哪些安全要求？ → A: 仅要求最小长度8字符（无额外复杂度要求）
- Q: 当玩家断线重连时，游戏状态如何同步？ → A: 重连时发送完整的游戏状态快照
- Q: 前端应该记录哪些级别的日志用于问题诊断？ → A: 记录错误和警告（包括API失败、WebSocket断线）
- Q: 当后端API版本升级导致不兼容时，前端如何处理？ → A: 假设前后端总是同步部署，不处理版本不兼容

**已移除功能**: 游戏内聊天系统已从规范中移除（玩家之间不需要聊天功能）
...
### User Story 3 - Game Action Submission (Priority: P1)
...
### User Story 4 - Turn Timeout Warning (Priority: P2)
...
### User Story 5 - WebSocket Event Handling Enhancement (Priority: P3)
...
### User Story 6 - Crime Scene Game Core Logic (Priority: P1)
...
- 当玩家在游戏进行中短暂断线后重连会怎样？系统应发送完整的游戏状态快照，确保客户端与服务器状态一致。
...
- **FR-004**: 系统必须验证密码长度至少8个字符（无其他复杂度要求）
...
- **FR-016**: 系统必须提供清晰的行动选择界面，包括调查、询问、指控等行动类型
- **FR-017**: 系统必须只允许当前回合玩家提交行动
- **FR-018**: 系统必须在提交行动后立即通过 WebSocket 发送给服务器
- **FR-019**: 系统必须在行动处理期间显示加载状态
- **FR-020**: 系统必须在收到服务器响应后更新游戏状态
- **FR-021**: 系统必须验证行动的有效性（例如不能重复调查同一地点）
- **FR-022**: 系统必须在行动失败时显示明确的错误信息
- **FR-023**: 系统必须实现回合倒计时功能，显示剩余时间
- **FR-024**: 系统必须在回合超时时自动跳过当前玩家

#### Turn Timeout Warning (P2)

- **FR-025**: 系统必须在剩余时间少于30秒时显示黄色警告提示
- **FR-026**: 系统必须在剩余时间少于10秒时显示红色闪烁警告
- **FR-027**: 系统必须在剩余时间少于10秒时播放提示音（可静音）
- **FR-028**: 系统必须在行动提交后立即清除超时警告
- **FR-029**: 系统必须在超时后显示"超时"提示并自动进入下一回合

#### WebSocket Event Handling (P3)

- **FR-030**: 系统必须正确处理 lobby_joined 和 lobby_left 确认事件
- **FR-031**: 系统必须在收到 game_state_update 事件时更新完整的游戏状态
- **FR-032**: 系统必须在收到 turn_changed 事件时高亮当前回合玩家
- **FR-033**: 系统必须在收到 ai_thinking 事件时显示 AI 思考动画
- **FR-034**: 系统必须在收到 ai_action 事件时展示 AI 的行动详情
- **FR-035**: 系统必须在收到 game_ended 事件时显示游戏结果界面
- **FR-036**: 系统必须为所有 WebSocket 事件实现错误处理和重试机制，重连后接收完整游戏状态快照
- **FR-037**: 系统必须记录错误和警告级别的日志，包括 API 调用失败、WebSocket 断线事件、游戏状态异常等

#### Crime Scene Game Logic (P1)

- **FR-038**: 系统必须在游戏开始时显示案件背景故事和基本信息
- **FR-039**: 系统必须显示所有可调查地点的列表，标记已调查和未调查状态
- **FR-040**: 系统必须显示所有嫌疑人的列表和基本信息
- **FR-041**: 系统必须在玩家调查地点后显示发现的线索
- **FR-042**: 系统必须记录玩家已收集的所有线索，可随时查看
- **FR-043**: 系统必须在玩家询问嫌疑人时显示对话界面和嫌疑人回答
- **FR-044**: 系统必须记录所有询问历史，可随时回顾
- **FR-045**: 系统必须提供推理界面，展示线索之间的关联
- **FR-046**: 系统必须提供指控界面，允许玩家选择嫌疑人并提供证据
- **FR-047**: 系统必须验证指控的正确性并给出即时反馈
- **FR-048**: 系统必须在游戏结束时显示完整的案件真相和解释
- **FR-049**: 系统必须记录每个玩家的行动历史和决策过程
...
- **SC-003**: 用户在回合超时前30秒和10秒时能看到明显的视觉警告
- **SC-004**: 90%的游戏行动在3秒内得到服务器响应并更新界面
- **SC-005**: 用户可以在5分钟内理解犯罪现场游戏的基本玩法和目标
- **SC-006**: 完整的犯罪现场游戏从开始到结束可以在30-60分钟内完成
- **SC-007**: 所有 WebSocket 事件的处理不会导致界面卡顿或延迟超过500毫秒
- **SC-008**: 用户在查看其他玩家资料时等待时间少于1秒
- **SC-009**: 房间列表筛选结果在500毫秒内显示
- **SC-010**: 系统支持至少100个并发用户同时使用所有功能而不出现性能下降
- **SC-011**: 85%的新用户能在首次游戏中成功完成至少一次调查和询问行动

## Assumptions

- 后端 API 已完全实现并且稳定运行（基于前期分析，后端接口已完备）
- 前后端总是同步部署，不存在API版本不兼容的情况
